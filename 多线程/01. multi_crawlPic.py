...
def save_img(img_src,file_path):
    """
...
@wrapper.log_end
def single_save_img(img_src_list,file_path,headers=headers):
    #print(img_src_list)
    for each in img_src_list:
        save_img(each,file_path,headers=headers)

@wrapper.log_start
@wrapper.calculate_time
@wrapper.log_end
def multi_save_img(img_src_list,file_path,headers=headers):
    """
    输入一个包含图片链接的列表，将这些图片保存到file_path输入的位置中。
    :param img_src_list:
    :param file_path:
    ...
    for url in img_src_list:
        thread_list.append(
            threading.Thread(target=save_img, args=(url,file_path,headers))
        )
...
if __name__ == '__main__':
    ...
    pic_urls=[]
    for url in urls:
        pic_urls += crawl_pic_url(url=url, headers=headers)
    #单线程爬取90张图片，耗时：5分45秒
    #single_save_img(pic_urls,file_path,headers)

    #多线程爬取90张图片，耗时：4分30秒。
    #多线程提速不明显，可能是因为线程开启太多，可以使用线程池和生产者&消费者框架试试。
    #线程池可以重复利用线程，减少了线程开启和关闭的时间
    #生产者&消费者框架可以减少爬取的时间。
    multi_save_img(pic_urls,file_path,headers)
